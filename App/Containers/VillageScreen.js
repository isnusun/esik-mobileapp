...
class VillageScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            items: [],
    ...
    openRw = param => {
        NavigationService.navigate('Rw', { param: param });
    }
    ...
    render() {
        return (
            <ScrollView style={styles.container}>
                ...
                <Text style={[Fonts.style.h2, { alignSelf: "center" }]}>Daftar Dusun</Text>
                <View style={styles.searchBox}>
                    <TextInput
                        style={styles.searchInput}
                        placeholder='Pencarian dusun'
                ...
                {this.props.isLoading ? (
                    <ActivityIndicator size="large" />
                        ...
                        <FlatList
                            style={{ width: '100%' }}
                            keyExtractor={(item, index) => item.dusun}
                            data={this.state.items}
                            renderItem={({ item, index }) => (
                                <TouchableOpacity onPress={() => this.openRw(item)}>
                                    <View style={styles.itemBox2}>
                                        <View style={styles.itemTextBox}>
                                            <Text style={styles.itemText}>Dusun: {item.dusun.toUpperCase()}</Text>
