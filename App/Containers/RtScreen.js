...
class RtScreen extends Component {
    constructor(props) {
        super(props);
    ...
    openRespondent = (dusun, rw, rt) => {
        NavigationService.navigate('Respondent', { dusun: dusun, rw: rw, rt: rt });
    }
    render() {
        return (
            <ScrollView style={styles.container}>
                <Loader loading={this.state.loading} />
                <Text style={[Fonts.style.h2, { alignSelf: "center" }]}>Daftar RT</Text>
                <Text style={[Fonts.style.h4, { alignSelf: "center" }]}>Dusun {this.state.dusun} Rw {this.state.rw}</Text>
                ...
                {this.props.isLoading ? (
                    <ActivityIndicator size="large" />
                        ...
                        <FlatList
                            ...
                            keyExtractor={(item, index) => item.rt}
                            data={this.state.items}
                            renderItem={({ item, index }) => (
                                <TouchableOpacity onPress={() => this.openRespondent(item.dusun, item.rw, item.rt)}>
                                    <View style={styles.itemBox2}>
                                        <View style={styles.itemTextBox}>
                                            <Text style={styles.itemText}>Rt: {item.rt}</Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(RtScreen)
