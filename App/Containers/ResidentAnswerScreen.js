...
import { TouchableOpacity, ActivityAnswer, Image, FlatList, ScrollView, Text, TextInput, View } from 'react-native'
...
class ResidentAnswerScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            total: 0,
    ...
    _getRt = () => {
        ...
        residentAnswerDb.initDB().then(() => {
            residentAnswerDb.totalResidentAnswer(this.props.user.userid, this.state.filter)
                .then(total => {
                    this.setState({
                        total: total,
                    ...
                    if (total === 0) {
                        ...
                        let _rt = residentAnswerDb.listResidentAnswer(this.props.user.userid, this.state.filter, this.state.per_page, (this.state.page - 1) * this.state.per_page)
                        _rt.then((items) => {
                            this.setState({
                                loading: false,
                                items: items
                            ...
                            setTimeout(() => {
                                residentAnswerDb.closeDatabase()
                            }, 100)
                        }).catch(() => {
                            this.setState({
                                loading: false
    ...
    render() {
        return (
            <ScrollView style={styles.container}>
                ...
                <Text style={[Fonts.style.h1, { alignSelf: "center" }]}>Jawaban ART</Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(ResidentAnswerScreen)
